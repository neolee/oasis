...
import Oasis.Tui.State (AppState(..), Name(..), TuiEvent(..), mkState, defaultOutput)
...
main = do
  ...
  case mPath of
    Nothing -> do
      ...
      let st = mkState chan emptyCfg [] [] runnerNames defaultOutput "providers.toml not found"
      runTui chan st
    Just path -> do
      ...
      case cfgResult of
        Left err -> do
          let emptyCfg = Config mempty (Defaults "" "") mempty
          let st = mkState chan emptyCfg [] [] runnerNames defaultOutput ("Failed to load config: " <> err)
          runTui chan st
        Right cfg -> do
          let providerNames = sort (M.keys (providers cfg))
          let st = mkState chan cfg providerNames [] runnerNames defaultOutput ("Loaded providers from " <> toText path)
          runTui chan st
  where
    runTui chan st = do
